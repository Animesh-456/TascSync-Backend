...
// const viewtaskPending = async (userid, status, pageNumb) => {

//   const pageNumber = pageNumb;
//   const pageSize = 10;
...
// const viewtaskComplete = async (userid, status, pageNumb) => {
//   // let tasks = await task.find({ status: status.toLowerCase(), assignedTo: userid }).sort({ createdAt: -1 }).populate({
...
//   var ty = await task.findByIdAndUpdate({ _id: tk.id }, { $set: { title: tk.title, description: tk.description } }, { new: true })
...
//   await task.findByIdAndDelete({ _id: tk.id })
// }
